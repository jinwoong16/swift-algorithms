...
func deapthFirstSearch<T>(_ graph: AbstractGraph<T>, source: Vertex<T>) -> [T] {
  ...
  while !stack.isEmpty {
    if let node = stack.pop() {
      if !visited[node, default: false] {
        visited[node] = true
        nodesExplored.append(node.data)
      }
      
      for edge in graph.edgesFrom(node).reversed() {
...
func testDFSWithListGraph() {
  ...
  let nodesExplored = deapthFirstSearch(graph, source: a)
  print(nodesExplored)
}

func testDFSWithMatrixGraph() {
  ...
  let matrixNodesExplored = deapthFirstSearch(matrix, source: ma)
  print(matrixNodesExplored)
  
  print(matrix.edgesFrom(me))
}
